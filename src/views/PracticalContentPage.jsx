        ...
        if (!isFolder) {
            if (name.endsWith('.pdf')) {
                ...
                htmlContent = `<div class="pdf-container" style="height:80vh;">
                    <iframe src="${escapeHtml(`https://docs.google.com/viewer?url=${encodeURIComponent(rawFileUrl)}&embedded=true`)}"
                        style="width:100%;height:100%;border:none;"></iframe>
                </div>`;
            } else if (name.endsWith('.png') || name.endsWith('.jpg') || name.endsWith('.jpeg') || name.endsWith('.gif') || name.endsWith('.webp')) {
                const encodedItemPath2 = itemPath.split('/').map(encodeURIComponent).join('/');
                htmlContent = `<div style="display:flex;justify-content:center;padding:2rem;">
                    <img src="https://raw.githubusercontent.com/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/main/${encodedItemPath2}"
