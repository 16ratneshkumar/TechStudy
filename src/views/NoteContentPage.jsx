...
// Escapes values interpolated into raw HTML strings (htmlContent template literals)
...
export default async function NoteContentPage({ params, searchParams }) {
    ...
    try {
        if (!name.includes('.')) {
            ...
            const rawFileUrl = `https://raw.githubusercontent.com/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/main/${encodedItemPath}`;
            const viewerUrl = `https://docs.google.com/viewer?url=${encodeURIComponent(rawFileUrl)}&embedded=true`;

            // Security: rawFileUrl and viewerUrl are built from static-config owner/repo
            // and URL-encoded itemPath — safe to use in href/src attributes
            htmlContent = `
            <div style="display:flex; justify-content:flex-end; margin-bottom:1rem;">
                <a href="${escapeHtml(rawFileUrl)}" download target="_blank" rel="noopener noreferrer" class="btn btn-primary" style="text-decoration:none; display:inline-flex; align-items:center; gap:0.5rem;">
            ...
            htmlContent = `<div style="display:flex;justify-content:center;padding:2rem;">
                <img src="https://raw.githubusercontent.com/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/main/${itemPath.split('/').map(encodeURIComponent).join('/')}"
                    alt="${escapeHtml(name)}" style="max-width:100%;height:auto;border-radius:8px;" />
            </div>`;
